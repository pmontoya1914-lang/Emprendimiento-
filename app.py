...
st.subheader("Este instrumenmto permite saber el nivel de desarrollo y madurez de tu emprendimiento en seis áreas clave: administrativa, comercial, financiera, técnica, ecológica y legal. No existen respuestas correctas o incorrectas. Responde con honestidad para obtener un diagnóstico útil.")
...
tipo_negocio = st.text_input("Tipo de negocio")

municipio = st.text_input("Municipio")

total_admin = 0
import streamlit as st
total_admin += st.selectbox("1. ¿Cuenta con estructura organizacional definida?", [0,1,2,3,4,5])
total_admin += st.selectbox("2. ¿Están claros los roles y responsabilidades?", [0,1,2,3,4,5])
total_admin += st.selectbox("3. ¿Existe un responsable de decisiones clave?", [0,1,2,3,4,5])
total_admin += st.selectbox("4. ¿Los socios tienen funciones diferenciadas?", [0,1,2,3,4,5])
total_admin += st.selectbox("5. ¿Las decisiones se documentan?", [0,1,2,3,4,5])
total_admin += st.selectbox("6. ¿Existen procesos definidos para actividades clave?", [0,1,2,3,4,5])
total_admin += st.selectbox("7. ¿Los procesos están documentados?", [0,1,2,3,4,5])
total_admin += st.selectbox("8. ¿Se usan herramientas de seguimiento de tareas?", [0,1,2,3,4,5])
total_admin += st.selectbox("9. ¿Se mide el desempeño periódicamente?", [0,1,2,3,4,5])
total_admin += st.selectbox("10. ¿Existen indicadores administrativos?", [0,1,2,3,4,5])
total_admin += st.selectbox("11. ¿Tiene objetivos claros a corto y mediano plazo?", [0,1,2,3,4,5])
total_admin += st.selectbox("12. ¿Realiza planeación anual o semestral?", [0,1,2,3,4,5])
total_admin += st.selectbox("13. ¿Revisa resultados contra lo planeado?", [0,1,2,3,4,5])
total_admin += st.selectbox("14. ¿Ajusta estrategias cuando algo no funciona?", [0,1,2,3,4,5])
total_admin += st.selectbox("15. ¿El negocio depende de una sola persona?", [0,1,2,3,4,5])
total_admin += st.selectbox("16. ¿Cuenta con personal formal?", [0,1,2,3,4,5])
total_admin += st.selectbox("17. ¿Hay claridad en horarios y pagos?", [0,1,2,3,4,5])
total_admin += st.selectbox("18. ¿Se capacita al personal regularmente?", [0,1,2,3,4,5])
total_admin += st.selectbox("19. ¿Existe alta rotación de personal?", [0,1,2,3,4,5])
...
total_comercial += st.selectbox("21. ¿Tiene definida su propuesta de valor?", [0,1,2,3,4,5])
total_comercial += st.selectbox("22. ¿Resuelve un problema real del cliente?", [0,1,2,3,4,5])
total_comercial += st.selectbox("23. ¿Tiene identificado a su cliente objetivo?", [0,1,2,3,4,5])
total_comercial += st.selectbox("24. ¿Conoce el perfil del cliente ideal?", [0,1,2,3,4,5])
total_comercial += st.selectbox("25. ¿Se diferencia de la competencia?", [0,1,2,3,4,5])
total_comercial += st.selectbox("26. ¿Cuenta con canales de venta definidos?", [0,1,2,3,4,5])
total_comercial += st.selectbox("27. ¿Los canales actuales son efectivos?", [0,1,2,3,4,5])
total_comercial += st.selectbox("28. ¿Existe un proceso de ventas estructurado?", [0,1,2,3,4,5])
total_comercial += st.selectbox("29. ¿Mide la conversión de prospectos?", [0,1,2,3,4,5])
total_comercial += st.selectbox("30. ¿Da seguimiento a clientes potenciales?", [0,1,2,3,4,5])
total_comercial += st.selectbox("31. ¿El modelo de ingresos está definido?", [0,1,2,3,4,5])
total_comercial += st.selectbox("32. ¿Los precios cubren costos y generan utilidad?", [0,1,2,3,4,5])
total_comercial += st.selectbox("33. ¿Revisa precios periódicamente?", [0,1,2,3,4,5])
total_comercial += st.selectbox("34. ¿Depende de pocos clientes?", [0,1,2,3,4,5])
total_comercial += st.selectbox("35. ¿Los ingresos son recurrentes?", [0,1,2,3,4,5])
total_comercial += st.selectbox("36. ¿Realiza acciones de marketing constantes?", [0,1,2,3,4,5])
total_comercial += st.selectbox("37. ¿Utiliza medios digitales para vender?", [0,1,2,3,4,5])
total_comercial += st.selectbox("38. ¿Mide la satisfacción del cliente?", [0,1,2,3,4,5])
total_comercial += st.selectbox("39. ¿Cuenta con estrategias de fidelización?", [0,1,2,3,4,5])
total_comercial += st.selectbox("40. ¿La marca es reconocible en su mercado?", [0,1,2,3,4,5])
